...
class DeepResearchFlow(Flow[ResearchState]):
    ...
    def analyze_query(self):
        ...
        prompt = (# Write the prompt for the LLM to decide if the query is simple or requires research 
                  "Determine whether this query requires deep research or can be answered simply.\n\n"
    ...
    def simple_answer(self):
        ...
        prompt = (# Write the missing part of the query for the LLM 
                 "Provide a concise and accurate answer to the following query.\n\n"
                 f"Query: \"{self.state.user_query}\"\n\n"
                 "Answer:"
                 )
        # set up the LLM
...
def kickoff():
    ...
    deep_research_flow = DeepResearchFlow(enable_tracing=True)
    ### END CODE HERE ###
